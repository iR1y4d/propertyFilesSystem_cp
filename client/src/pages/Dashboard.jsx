...
const Dashboard = () => {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-2xl font-bold text-gray-800">مرحباً، {user?.firstName}</h1>
        <p className="text-gray-500">نظرة عامة على النظام اليوم</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {cards.map((card, index) => (
          <div 
            ...
            className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex items-center gap-4 cursor-pointer hover:shadow-md hover:border-blue-200 transition-all duration-200 group"
          >
            <div className={`${card.color} p-4 rounded-lg text-white shadow-lg group-hover:scale-110 transition-transform duration-200`}>
              <card.icon size={24} />
            </div>
            <div>
